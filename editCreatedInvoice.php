    ...
    <div class="box box-primary">
     <div class="box-body">
      <form role="form" id="invoiceForm" method="POST" action="InvoiceController.php">
       ...
       <div class="row old-services<?php echo $i; ?>">
           <div class="col-md-4">
               <div class="form-group">
       ...
       <?php 
           ...
           $net_amount_after_row_remove = $invoiceDetails['net_amount'] - $amount;
       ?>
       <?php if( $i > 0 ) { ?>
        <div class="row old-services<?php echo $i; ?>">
         <div class="col-md-12 text-right">
          <a class="remove-old-div btn btn-warning" onclick="removeOldDiv('<?php echo $i; ?>', '<?php echo $invoiceManager->service_id[$i];?>','<?php echo $invoice_id; ?>', '<?php echo $net_amount_after_row_remove;?>');">Remove</a>
         </div>
        </div>
       <?php } ?>
       ...
       <input id="client_id" type="hidden" name="client_id" value="<?php echo $invoiceDetails['client_id'] ?>">
...
<script type="text/javascript">
    ...
    // end
    // delete old services row 
    function removeOldDiv(row_id, service_id, invoice_id, net_amount_after_row_remove) {
     ...
     function(){
         $.ajax({
             ...
             cache: false,
             data: {
              "action": 'delete_old_services',
              "old_service_id": service_id, 
              "old_invoice_id": invoice_id,
              "net_amount_after_row_remove": net_amount_after_row_remove 
             },
   ...
   // send additional emails CC
    ...
    $('#sendAddEmailCCBtn').click(function() {
        if(sendAddEmailCount <= 10) {
            $('#sendAdditionalEmailDiv').append('<div class="form-group sendAddEmailInput" id="sendAddEmailInput'+sendAddEmailCount+'"><input type="email" placeholder="Enter email for CC" name="sendAdditionalEmails[]" class="form-control" required><br><a onclick="removeAddEmailInput('+sendAddEmailCount+');" class="btn btn-danger btn-sm">Delete</a></div>');
            sendAddEmailCount++;
         } else {
